...
// Multiple levels
page.getByRole('table')
  .getByRole('row').filter({ hasText: 'Product A' })
...
await page.getByRole('row', { name: /john@example.com/ })
...
page.getByRole('row').filter({ hasText: 'Product A' }).getByRole('button').first()
