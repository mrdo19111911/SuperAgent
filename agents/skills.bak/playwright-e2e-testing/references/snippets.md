  ...
  captureLogs: async ({ page }, use, testInfo) => {
    ...
    page.on('response', async (resp) => {
      ...
      if (timing.responseEnd - timing.startTime > 1000) {
        consoleMessages.push(`[slow-api] ${resp.url()} took ${timing.responseEnd - timing.startTime}ms`);
      }
    });
...
test('verify request body and response', async ({ page }) => {
  ...
  const requestPromise = page.waitForRequest((r) => r.url().includes('/api/preferences'));
  const responsePromise = page.waitForResponse((r) => r.url().includes('/api/preferences'));
